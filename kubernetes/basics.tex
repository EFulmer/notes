% ============================================================
%  Kubernetes Notes
%  Compiled with pdfLaTeX (TeXShop on macOS)
% ============================================================
\documentclass[11pt, a4paper]{article}

% --- Packages ---
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{lmodern}           % cleaner fonts
\usepackage[margin=2.5cm]{geometry}
\usepackage{parskip}           % space between paragraphs instead of indent
\usepackage{hyperref}          % clickable links & TOC
\usepackage{xcolor}            % colour support
\usepackage{listings}          % code / YAML blocks
\usepackage{booktabs}          % nicer tables
\usepackage{enumitem}          % control over lists

% --- hyperref setup ---
\hypersetup{
    colorlinks = true,
    linkcolor  = blue!60!black,
    urlcolor   = blue!60!black,
    citecolor  = green!50!black,
    pdftitle   = {Kubernetes Notes},
    pdfauthor  = {Eric R. Fulmer},
}

% --- listings setup (for YAML / shell snippets) ---
\lstdefinestyle{shell}{
    backgroundcolor = \color{gray!10},
    basicstyle      = \ttfamily\small,
    breaklines      = true,
    frame           = single,
    framerule       = 0.4pt,
    rulecolor       = \color{gray!50},
    numbers         = none,
    xleftmargin     = 6pt,
    xrightmargin    = 6pt,
}
\lstset{style=shell}

% ============================================================
\begin{document}

% --- Title block ---
\begin{center}
    {\LARGE \textbf{Kubernetes Notes}} \\[0.5em]
    {\large Personal Reference} \\[0.3em]
    {\small Last updated: \today}
\end{center}

\tableofcontents
\bigskip
\hrule
\bigskip

% ============================================================
\section{Introduction}

The purpose of this document is, obviously, my Kubernetes notes for myself. I am trying to avoid buzzwords and jargon, and explain both the \textit{hows} and \textit{whys} of Kubernetes. 

That implies two basic questions worth answering before we go any further:

\begin{itemize}
\item What \textit{is} Kubernetes?
\item Why does Kubernetes exist -- and why have so many people and organizations found it useful?
\end{itemize}

You'll often see Kubernetes described as "the operating system of the cloud." This is a buzzy way of describing it, but it is an accurate description. The different components of a Kubernetes cluster facilitate deploying applications in a cloud environment. That cloud may be public, like Amazon Web Services, or private, like your employer or school's server racks. 

Software suited for running on a cloud should be durable and largely agnostic to what computer it runs on, aside from fundamental incompatibilities on the level of x86 vs. ARM processor architectures, Linux vs. Windows vs. macOS, and so on. As operating systems provide convenient interfaces to the physical computer programs run on, they (ideally!) remove a programmer's need to know the specific CPU, RAM, and networking details of the machine their code runs on. Kubernetes does the same one level up -- abstracting away individual servers, nodes, regions, and storage types, handling concerns like application self-healing, upgrades and rollbacks, and storage on the programmer's behalf. This may not be a perfect analogy, but none is. It is the springboard for understanding those \textit{hows} and \textit{whys}.

Kubernetes was originally developed at Google, to make it easier for programmers to deploy and maintain programs at so-called "Google scale;" highly available, highly fault-tolerant systems. It is now owned and maintained by the Cloud Native Computing Foundation (CNCF). Before Kubernetes' release, containerization tools -- Docker being the most known -- were gaining notice and popularity for deploying industry applications. Docker handles a lot of the repeated complexities of deploying applications; packaging up everything you need to run the application, with a smaller footprint than a VM. However, Docker and Podman are \textit{containerization} tools. Their purpose is to set up a lightweight-but-isolated environment to run an application in. Things like application auto-healing and rollouts are not in scope for those programs. While Docker provides related tools for some of these, specifically Docker Compose and Docker Swarm, a solution more suitable for production deployments was still needed. Enter Kubernetes.

As well, since Docker is a commercial product, the Open Container Initiative was established; an organization to establish and govern open standards for containerization technologies.  

% ============================================================
\section{Components of a Kubernetes Cluster}

A Kubernetes cluster consists of a \textit{control plane node} (the "leader," "brain," or "main office" of the cluster) and one or more \textit{worker nodes}. The control plane manages the desired state of the cluster; worker nodes run the actual workloads. Smaller, lower-stakes deployments like home labs or other sorts of non-production Kubernetes clusters, may run a single node which hosts both the control plane's components and runs workloads, but this isn't suitable for production use. Conversely, high availability setups may run multiple instances of some or all of the Control Plane components for redundancy.

\subsection{API Server}

\textem{This section is a work in progress!}

The API server exposes the Kubernetes API, which is the way for external services to communicate with the cluster. The API is served over HTTPS; all \texttt{kubectl} commands are essentially wrappers around it, handling authentication, resource selection, and basically everything aside from sending and receiving the requests and responses itself.

\subsubsection{More details on the API Server}

\textem{This section is a work in progress!}

Endpoints follow the format of \texttt{apis/{version}/{api}/...}

\subsection{\texttt{etcd}}

A core component of Kubernetes is the durable \texttt{etcd} database. \texttt{etcd} is a key-value database (as opposed to a relational/SQL database, or a document store like MongoDB) used by many other distributed systems. \texttt{etcd} is a Kubernetes cluster's "source of truth" for the \textit{desired cluster state}; a concept that is crucial to understanding and using Kubernetes. 

Like most well-engineered pieces of software, access to the database is not a free-for-all affair. The Kubernetes API server is the intermediary between the \texttt{etcd} database and the outside world. The rest of this subsection discusses \texttt{etcd} and how Kubernetes uses it in greater depth; readers for whom this is not interesting or relevant can skip it. 

High-availability Kubernetes servers may want to run multiple \texttt{etcd} instances; I will go into more detail on this later. Clusters using multiple \texttt{etcd}s use the \textit{raft consensus algorithm} to designate a "leader" or main instance. 

\texttt{etcd} consists of three executables:

\begin{enumerate}
\item \texttt{etcd} - the program running the database itself.
\item \texttt{etcdctl} - provides a command-line interface for querying the database and handling admin tasks
\item \texttt{etcdutl} - used for tasks that require operating directly on \texttt{etcd} data files, like migrating data between versions of \texttt{etcd}, restoring snapshots, defragmenting the database, and database validation
\end{enumerate}

\subsubsection{How Kubernetes stores data in \texttt{etcd}}

\texttt{etcd} is a key-value database. Possible values for keys are seemingly pretty free-form, but Kubernetes uses a Linux path-like format for them. All keys start with \texttt{/registry/}, followed by the resource type, the namespace (if the object is namespaced), then the resource's name.

For example, the default namespace's definition is at \texttt{/registry/namespaces/default/}, and namespaced objects like Pods will live at, e.g. \texttt{/registry/pods/myAppNS/apiPod/}.

\subsubsection{Using \texttt{etcdctl}}

In most setups, \texttt{etcdctl} will require you to specify the API version, the endpoint and port the database server is listening at, and the certificate info. For a concrete example, if you're running a cluster locally using Docker Desktop, from within the Pod running \texttt{etcd}, your \texttt{etcdctl} commands may look something like

\begin{lstlisting}
ETCDCTL_API=3 etcdctl get /registry/namespaces/default \
  --endpoints=https://127.0.0.1:2379 \
  --cacert=/etc/kubernetes/pki/etcd/ca.crt \
  --cert=/etc/kubernetes/pki/etcd/server.crt \
  --key=/etc/kubernetes/pki/etcd/server.key
\end{lstlisting}

The values for the Kubernetes objects are stored in Protobuf format, so while this command should run successfully (at least on a cluster created by Docker Desktop), the output will not be human-readable. There are multiple ways to turn the Protobuf data into more readable formats, but the primary one is, of course, the Kubernetes API. 

To view the values in a human-readable form without leaving \texttt{etcdctl}, pass the \texttt{-w} flag (for "write," as in, write out to your terminal/display). Options include \texttt{fields}, \texttt{simple}, and \texttt{json}. \texttt{fields} and \texttt{json} will render the values for individual keys in a human-readable format, in my experience.

\subsubsection{Commands and Flags}
Here are some \texttt{etcdctl} commands and flags worth knowing:

\begin{itemize}
\item \texttt{etcdctl get|put|del} - see, set, or delete entries.
\item \texttt{etcdctl get {-}{-}prefix} - get all items whose key starts with the given prefix.
\item \texttt{etcdctl get {-}{-}keys-only} - get only the key, not the value, if it exists. Useful in conjunction with \texttt{{-}{-}prefix} to get every object of a specific type, in a specific namespace, or both.
\end{itemize}

\paragraph{Further reading}

\begin{itemize}
\item https://etcd.io/docs/v3.6/
\end{itemize}

\subsection{Controller Manager}

The \textit{controller manager} is a daemon that manages controllers. This implies a very obvious question -- what is a controller? Controllers programs that monitor the cluster (through the API server, Kubelet, or both?) to it to reconcile the desired and actual state of your cluster. Each controller manages a type or type of objects. For example, the \textit{Node manager} checks each Node's health every five seconds. If a Node goes down, with a grace period of forty seconds, the Node manager recreates its' Pods on another node. 

Additional controllers for Custom Resource Definitions (CRDs) can be added. 

\subsection{Scheduler}

The \textit{scheduler} is the service that exposes

\subsection{Container Runtime}

Every Node, worker or Control Plane, must have at least one \textit{container runtime} installed on it. Container runtimes, such as Docker Engine, handle the crucial job of actually running containers. While Docker Engine is the most well-known, it can be heavyweight for some deployments. Other runtimes include containerd, CRI-O, and Mirantis Container Runtime, which is a commercially available runtime.

All container runtimes should be compatible with the Container Runtime Interface (CRI), which is an interface or protocol for a uniform method of communication between the kubelet and container runtime. 

CLI applications to interact with runtimes include \texttt{ctr}, \texttt{nerdctl}, and \texttt{crictl}. \texttt{crictl} is used for debugging purposes, \texttt{nerdctl} is a general-purpose Docker-like CLI application specifically for containerd, and \texttt{crictl} is another debugging-focused tool for all CRI-compatible runtimes, associated with Kubernetes.

Each of these applications connects to Unix sockets suck as:

\begin{itemize}
\item \texttt{unix:///var/run/dockershim.sock}
\item \texttt{unix:///var/run/containerd/containerd.sock}
\item \texttt{unix:///var/run/crio/crio.sock}
\item \texttt{unix:///var/run/cri-dockerd.sock}
\end{itemize}

\subsubsection{Common Container Runtime Commands}

Coming soon!

\subsection{Kubelet}

The kubelet is an application that runs on each Node, with three major responsibilities:

\begin{enumerate}
\item registering the Node with the Control Plane upon Node start-up
\item managing communication between the Node and the Control Plane
\item managing Pods so that the container runtime can run their constituent containers, by doing things like:
	\begin{enumerate}
	\item mounting Volumes, ConfigMaps, Secrets
	\item running liveness and readiness probes, to monitor Pod health
	\item checking they respect any resource limits defined for them
	\end{enumerate}
\end{enumerate}

The kubelet communicates with the Control Plane primarily through the API Server, watching for any updates to PodSpecs (the specification component of a Pod manifest) scheduled to its specific Node. It also posts updates back to the API Server about the \textit{actual state} of those \textit{desired resources}.

The kubelet also runs its own small HTTPS API, through which it proxies requests that operate on the container-level, like \texttt{kubectl logs} and \texttt{kubectl exec}.

Aside from changes to the cluster state requested through \texttt{kubectl}, which are assigned to a Node by the scheduler and communicated to the kubelet via the API Server, the kubelet can also manage \textit{static Pods}. More on those later.

The kubelet is typically run as a daemon with \texttt{systemctl}, since it cannot manage itself as a Pod.

\subsection{Kube-Proxy}

Coming soon!

% ============================================================
\section{Objects in Kubernetes}

\subsection{Pods}

The Pod is the minimal "unit of work" in Kubernetes. Pods are a wrapper around containers, managing the basics of provisioning and running containers. This includes things like storage and networking.

A Pod can have multiple containers. The typical patterns for multi-container Pods are \textit{init containers}, where a container does some preliminary work before the actual computational work the Pod is intended for, like waiting for a database or some other external dependency to become available. \textit{Sidecar containers} run alongside the container that executes the primary work. An example use case for sidecar containers is a static site that renders and serves Markdown files in a Git repo. The sidecar pod can routinely fetch the latest Markdowns from Git and save developers from having to implement that desired behavior interleaved with the rest of the static site's code. 

While Pods are a crucial "primitive" in Kubernetes, much of the magic, like smart horizontal and vertical scaling, or automatically restarting crashed Pods, uses other objects like Deployments and Services that operate on Pods via \textit{selectors}{-}{-}more on them in their dedicated sections.  

\subsubsection{Multi-Container Pods}

\subsubsection{Static Pods}

\textit{Static Pods} are created directly on a desired Node by putting their manifest YAMLs in a dedicated path on the Node{-}{-}so without \texttt{kubectl create}, \texttt{apply}, or \texttt{run}. The Node's Kubelet will always automatically restart a static Pod if it crashes. Because static Pods are defined locally on the Node rather than through interacting with the API server, their existence is independent of it and the scheduler, making them resilient to any errors with those Control Plane components.

The standard path for static Pod manifests is \texttt{/etc/kubernetes/manifests/}; however, this can be set to whatever you like using either the \texttt{--pod-manifest-path} flag passed to the Kubelet, or in the Kubelet's config file as \texttt{staticPodPath}. The standard path for the Kubelet config file is \texttt{/var/lib/kubelet/config.yaml}, but you can always \texttt{ps aux | grep kubelet} for it. 

The Kubelet watches both the static Pod path and the API server. If a Pod is created via putting a manifest in the static Pod path, the static Pod is reported to the API server, has its Node's name appended to the name set in the manifest, and cannot be deleted through things like \texttt{kubectl} or the Kubernetes HTTP API.

One use case for static Pods is creating Pod manifests for each Control Plane component, and storing them in a Node's static Pod path. This is, in fact, how \texttt{kubeadm} sets up and manages clusters.

It is also worth noting that both static Pods and DaemonSets are ignored by the scheduler. For DaemonSets, each Node has an instance of the Pod. DaemonSets are elucidated upon in their dedicated section. 

\subsubsection{Manifests}

\textit{Manifests} are YAML files defining a Kubernetes resource. A Pod's manifest looks something like:

\begin{lstlisting}
apiVersion: v1
kind: Pod
metadata:
	# defined later
spec:
	# defined later
\end{lstlisting}

\subsection{Namespaces}

The Namespace is the simplest way to manage resource and workflow isolation in a Kubernetes cluster. All Kubernetes object types are either "namespaced" or not; this determines whether they are visible across the entire cluster or only to objects in their Namespace. Objects like ConfigMaps, Pods (and anything that has Pods, like Deployments, Jobs, and CronJobs), and Secrets are namespaced. Meanwhile Nodes and Volumes are not namespaced. A solid rule of thumb is that objects that correspond closely to to either physical hardware or cluster-wide information are probably not namespaced; however you can always check whether an object type is namespaced by running \texttt{kubectl api-resources} to get a list of all object types available to the cluster of your current context.

On cluster startup, Kubernetes creates three Namespaces:

\begin{itemize}
\item \texttt{default}
\item \texttt{kube-system}
\item \texttt{kube-public}
\end{itemize}

\subsection{Deployments}

... ReplicaSets and ReplicationControllers 

\subsection{DaemonSets}

A DaemonSet runs exactly one Pod on each Node of a cluster.

\subsection{Services}

Services handle networking in Kubernetes.

% ============================================================
\section{Storage}

Kubernetes, as with all container tools, works with a stateless paradigm. Pods (and therefore implicitly containers) should be edited sparingly, because the purpose of Kubernetes is to deploy, scale, and self-heal applications without need tor additional setup beyond the cluster existing. But state is needed. There are many obvious uses for state--mutating the state of the file system with logs, the contents of a database, etc.--and using Kubernetes does not remove the need to manage that state.

Kubernetes has tools for handling state. This section discusses how containers handle state, how to define, manage, and persist state for applications deployed in Kubernetes, and how to integrate with cloud storage providers like Amazon's S3. 

\subsection{Images, Containers and State}

Each line or command in a Dockerfile adds a new \textit{layer} to the image. Each layer is a specific change to the filesystem defined by the layer created by the last line. 

For an example from one of my own Dockerfiles:

\begin{lstlisting}
COPY pyproject.toml poetry.lock /app
RUN pip install --upgrade pip && \
    pip install poetry && \
    poetry config virtualenvs.create false && \
    poetry install --no-root

COPY . /app
\end{lstlisting}

The \texttt{COPY} commands define a new layer with the files from the host filesystem in the desired directory of the image's filesystem. The \texttt{RUN} commands execute their arguments and create a new layer storing the changes those commands made--in this example, those being to the Python installation in the image's filesystem, including changes to the system Python in the image, the \texttt{site-packages} directory which stores third-party libraries installed by \texttt{pip}, and so on.

\subsection{Volumes}

WIP.

Volume types include images, Secrets, ConfigMaps, and

\subsubsection{Secrets}

\subsubsection{ConfigMaps}

\subsubsection{Images as Volumes}

You can mount an OCI compatible image as a read-only Volume in a Pod. For an example:

\begin{lstlisting}
# ... Assume the rest of a valid Pod manifest is included ...
  volumes:
    - name: volume
      image:
        reference: registry.iximiuz.com/welcome-page:v1
        pullPolicy: IfNotPresent
\end{lstlisting}

% ============================================================
\section{\texttt{Kubectl}}

\texttt{kubectl} is the primary method for interacting with a Kubernetes cluster.

\subsection{Imperatively Working with \texttt{kubectl}}

\subsection{Declaratively Working with \texttt{kubectl}}

\subsection{JSON Path}

% ============================================================
\section{Kustomize}

% ============================================================
\section{Helm}

% ============================================================
\section{Cookbook}

This section contains tips and guides on how to handle common tasks related to maintaining Kubernetes clusters, and managing and debugging workloads deployed on them.

\subsection{Shelling into a Pod}

\subsection{Upgrading a Cluster}

These instructions are primarily for Debian-based distros using \texttt{apt}.

\subsubsection{Clusters Provisioned with \texttt{kubeadm}}

We can break upgrading a \texttt{kubeadm} cluster into two parts: upgrading the Control Plane node(s) and upgrading the workers. 

\paragraph{Control Plane}\mbox{}

You first need to check which package repository your OS is using; there is a separate repository for every minor version of Kubernetes. Ensure it's set to the minor release of Kubernetes you want to use. 

On Debian and Ubuntu, this is stored at \texttt{/etc/apt/sources.list.d/kubernetes.list}.

On RHEL, Fedora, and CentOS, it should be located at \texttt{/etc/yum.repos.d/kubernetes.repo}.

Regardless of your distro, in the file, the package repo URL should match a pattern similar to:

\begin{lstlisting}
https://pkgs.k8s.io/core:/stable:/v1.36
\end{lstlisting}

Change the version to the minor version you're upgrading to. 

Once the package repo is updated, instruct the package manager to update its package database with an \texttt{apt update} and then identify the specific patch version you wish to upgrade to with \texttt{apt-cache madison kubeadm}. \texttt{apt-cache madison} searches the local \texttt{apt} cache for packages matching the and prints info about them to stdout. Following is the output I got from \texttt{sudo apt-cache madison kubeadm}:

\begin{lstlisting}
kubeadm | 1.36.3-1.1 | https://pkgs.k8s.io/core:/stable:/v1.36/deb  Packages
kubeadm | 1.36.2-2.1 | https://pkgs.k8s.io/core:/stable:/v1.36/deb  Packages
kubeadm | 1.36.1-1.1 | https://pkgs.k8s.io/core:/stable:/v1.36/deb  Packages
kubeadm | 1.36.0-1.1 | https://pkgs.k8s.io/core:/stable:/v1.36/deb  Packages
\end{lstlisting}

Temporarily unhold the Control Plane's \texttt{kubeadm}, update it to your desired new version, and verify:

\begin{lstlisting}
sudo apt-mark unhold kubeadm && \
sudo apt-get update && \
sudo apt-get install -y kubeadm='1.36.3-1.1' && \
sudo apt-mark hold kubeadm
kubeadm version
\end{lstlisting}

With your Control Plane's new version of \texttt{kubeadm} installed, plan the upgrade with \texttt{sudo kubeadm upgrade plan}. If all is well, run the upgrade: \texttt{sudo kubeadm upgrade apply v1.36.3}. The success message will look like this:

\begin{lstlisting}
[upgrade/successful] SUCCESS! Your cluster was upgraded to "v1.36.3". Enjoy!

[upgrade/kubelet] Now that your control plane is upgraded, please proceed with upgrading your kubelets if you haven't already done so.
\end{lstlisting}

If you're running a high availability setup with multiple Control Plane nodes, you'll need to repeat the preceding with a few slight differences. Update the apt repository, but instead of \texttt{kubeadm upgrade apply}, run \texttt{kubeadm upgrade node}. Additionally, you don't need to upgrade the CNI plugin again, since all CRDs, Deployments, etc., across the cluster are upgraded in one shot.

Before upgrading the Kubelet and \texttt{kubectl}, cordon the Node and drain it. Then temporarily unhold the packages for them to update to the same minor version you upgraded \texttt{kubeadm} to. Finally, use \texttt{systemctl} to restart the Kubelet:

\begin{lstlisting}
# Update the packages:
sudo apt-mark unhold kubelet kubectl && \
sudo apt-get update && \
sudo apt-get install -y kubelet='1.36.3-1.1'  kubectl='1.36.3-1.1' && \
sudo apt-mark hold kubelet kubectl
# Restart the service:
sudo systemctl daemon-reload
sudo systemctl restart kubelet
\end{lstlisting}

After all this, you can uncordon the node.

\paragraph{Workers}\mbox{}

Each worker node should be cordoned and drained to avoid disrupting anything running on it.

Once everything running on the Node has been moved, begin the upgrade process: 

\begin{enumerate}
\item Upgrade the Node's \texttt{kubeadm}.
\item Run \texttt{sudo kubeadm upgrade node} to upgrade its' Kubelet's config,.
\item Upgrade \texttt{kubectl} and the Kubelet.
\item Restart the Kubelet service with \texttt{systemctl}).
\item Uncordon the Node.
\end{enumerate}

For more detailed explanations for any of these steps, reference the Control Plane section.

\subsubsection{Upgrading \texttt{etcd}}

\subsection{Backing up and restoring \texttt{etcd}}

\subsubsection{Backing up \texttt{etcd}}

\subsubsection{Restoring an \texttt{etcd} backup}

\subsection{Dealing with IP address exhaustion}

\end{document}